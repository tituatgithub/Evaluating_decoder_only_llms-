...
PROGRESS_PATH = os.path.join(OUTPUT_DIR, "progress_hi_hin_ro.json")
CHECKPOINT_PATH = os.path.join(OUTPUT_DIR, "llama_checkpoint_hi_hin_ro.json")
LIVE_STATUS_PATH = os.path.join(OUTPUT_DIR, "live_status_hi_hin_ro.json")
FINAL_OUTPUT_PATH = os.path.join(OUTPUT_DIR, "llama_translations_hi_hin_ro.json")
...
# ----------------------------
# Prompt template (Hindi in Romanized script)
# ----------------------------
def make_prompt(text, src_lang="Hinglish", tgt_lang="Hindi"):
    return f"""Aap ek upyogi anuvaad sahayak hain.
Nimnalikhit path ko {src_lang} se {tgt_lang} mein anuvaad kijiye.
...
def main():
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Translating"):
        ...
        generated = out[0]["generated_text"].split("Anuvaad:")[-1].strip()
        ...
        progress_record = {
            "Id": int(row["Id"]),
            "Input": row["Sentences"],
            "llama_translation_hi_hin_ro": generated
        }
        ...
        if (idx + 1) % 50 == 0:
            ...
            temp_df["llama_translation_hi_hin_ro"] = translations
            temp_df.to_json(CHECKPOINT_PATH, orient="records", lines=True, force_ascii=False)

    # Final save
    df["llama_translation_hi_hin_ro"] = translations
