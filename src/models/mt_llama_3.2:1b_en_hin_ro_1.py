...
PROGRESS_PATH = os.path.join(OUTPUT_DIR, "progress_hin_ro_en.json")
CHECKPOINT_PATH = os.path.join(OUTPUT_DIR, "llama_checkpoint_hin_ro_en.json")
LIVE_STATUS_PATH = os.path.join(OUTPUT_DIR, "live_status_hin_ro_en.json")
FINAL_OUTPUT_PATH = os.path.join(OUTPUT_DIR, "llama_translations_hin_ro_en.json")
...
# ----------------------------
# Prompt template (Hindi in Romanized script → English)
# ----------------------------
...
def main():
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Translating"):
        ...
        progress_record = {
            "Id": int(row["Id"]),
            "Input": row["Sentences"],
            "llama_translation_hin_ro_en": generated
        }
        ...
        if (idx + 1) % 50 == 0:
            ...
            temp_df["llama_translation_hin_ro_en"] = translations
            temp_df.to_json(CHECKPOINT_PATH, orient="records", lines=True, force_ascii=False)

    # Final save
    df["llama_translation_hin_ro_en"] = translations
